...
local testStr = ""
for i=1, 1 do
  testStr = testStr.."As mentioned above,there are many kinds of wireless systems other than cellular."
end

local fileName = "smallfull.txt"
local file = io.open(fileName, "rb")
local testStr = file:read("*all")
print(testStr:len())
...
local result
for i=1, 100 do
  result = lib:Compress(testStr)
end
--profiler.stop()
local out = io.open(fileName..".deflate", "wb")
local totalTime = os.clock()-time
print("compression time:", totalTime)
out:write(result)
out:close()
os.execute("rm -f "..fileName..".decompressed")
os.execute("puff -w "..fileName..".deflate > "..fileName..".decompressed")
local t = os.clock()
while(os.clock()-t < 1) do
end
local input = io.open(fileName..".decompressed", "rb")
local toBeCompare = input:read("*all")
assert(testStr:len() == toBeCompare:len(), "file size diff "..testStr:len().." "..toBeCompare:len())
for i=1, testStr:len() do
  assert(string_byte(testStr, i, i) == string_byte(toBeCompare, i, i), "Diff at location "..i.." "..string_byte(testStr, i, i).." "..string_byte(toBeCompare, i, i))
end



print("before compress", testStr:len(), "after compress", result:len())
local expected = {0x15, 0xca, 0xd1, 0x0d, 0x80, 0x20, 0x0c, 0x45, 0xd1, 0x55, 0xde, 0x00, 0xc6,
  0x1d, 0x1c, 0xa5, 0xca, 0x33, 0x10, 0x4b, 0x49, 0x68, 0xd5, 0xb0, 0xbd, 0xf8, 0x71, 0x7f, 0x4e,
  0xee, 0xe6, 0xa8, 0xb4, 0x28, 0xcd, 0x98, 0x20, 0x7b, 0x7b, 0xb8, 0x44, 0x66, 0x27, 0x64, 0x56,
  0xc5, 0x06, 0xae, 0x62, 0xc9, 0xd1, 0x4e, 0xbc, 0xa5, 0x53, 0xe9, 0x0e, 0x1f, 0x1e, 0xac, 0xd3,
  0xfe, 0x13, 0x91, 0xc5, 0x70, 0x50, 0xf5, 0x56, 0xe9, 0xeb, 0x07}
--assert(result:len() == #expected, "Size diff "..result:len().." "..#expected)
for i=1, #expected do
--assert(string_byte(result, i, i) == expected[i], "Diff at location "..i.." "..string_byte(result, i, i).." "..expected[i])
end
...
sort(copyHeap, function (a,b) return a[1] < b[1] end)
...
for i=1, N do
  ...
  table_insert(test, MinHeapPop(heap, heapSize))
...
for i=1, 1000 do
  ...
  local n = math.random(_end+1, 285)

  table_insert(dataTable, n)

end

local time = os.clock()

local symbolBitLength, symbolCode = GetHuffmanBitLengthAndCode(dataTable)
...
for i=0, MAX_SYMBOL do

  len = len..(symbolBitLength[i] or 0).." "
